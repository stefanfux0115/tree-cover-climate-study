...
for interpretable ML analysis - City Level
"""
import sys
import pandas as pd
import numpy as np
...
class FeatureSelector:
    """Comprehensive feature selection with visualizations for urban climate-energy ML - City Level"""
    
    def __init__(self):
        self.config = load_ml_analysis_config()
        self.data_dir = Path(__file__).parent.parent.parent / "data" / "processed" / "ml_analysis_city"
        self.output_dir = Path(__file__).parent.parent.parent / "output" / "feature_selection" / "city_level"
    ...
    def setup_logging(self):
        ...
        log_file = log_dir / f"city_feature_selection_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    ...
    def identify_feature_groups(self, df: pd.DataFrame) -> Dict[str, List[str]]:
        ...
        feature_groups = {
            ...
            'spatial': ['lon', 'lat', 'city_id'],
            'temporal': ['year', 'month', 'year_month'],
            'location': ['city_name', 'province'],
    ...
    def prepare_ml_data(self, train_df: pd.DataFrame, val_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series, pd.DataFrame, pd.Series]:
        ...
        exclude_features = [
            ...
            # Identifiers
            'city_id', 'city_name', 'province', 'year_month',
            # Other non-predictive features that might be city-specific
            'grid_count'  # This is a count variable, not predictive
        ]
        ...
        for col in feature_cols:
            if train_features[col].dtype in ['object', 'category']:
                # For categorical variables, use label encoding or drop them if they're not binary
                unique_vals = train_features[col].unique()
                if len(unique_vals) <= 10:  # Keep categorical variables with few categories
                    from sklearn.preprocessing import LabelEncoder
                    le = LabelEncoder()
                    # Combine train and val data for consistent encoding
                    combined_data = pd.concat([train_features[col], val_features[col]]).fillna('missing')
                    le.fit(combined_data)
                    train_features[col] = le.transform(train_features[col].fillna('missing'))
                    val_features[col] = le.transform(val_features[col].fillna('missing'))
                else:
                    # Drop categorical variables with too many categories
                    self.logger.warning(f"Dropping categorical feature with many categories: {col}")
                    train_features = train_features.drop(columns=[col])
                    val_features = val_features.drop(columns=[col])
    ...
    def visualize_feature_distributions(self, train_df: pd.DataFrame, feature_groups: Dict[str, List[str]]) -> None:
        ...
        # 2. Temperature bins distribution (if available)
...
def main():
    ...
    print(f"\nCity Feature Selection Completed Successfully!")
