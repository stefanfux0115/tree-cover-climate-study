"""
ML Data Loader - Load the already integrated grid_month_panel dataset
"""
import sys
import pandas as pd
import numpy as np
...
class MLDataLoader:
    """Load the integrated dataset for ML analysis"""
    ...
    def setup_logging(self):
        ...
        log_file = log_dir / f"ml_data_loader_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    ...
    def load_integrated_city_data(self) -> pd.DataFrame:
        """Load the already integrated city_month_panel dataset"""
        self.logger.info("Loading integrated city-level dataset...")
        
        # Load the pre-integrated dataset
        integrated_file = self.data_dir / "city_level" / "integrated" / "final" / "city_month_panel.csv"
        
        if not integrated_file.exists():
            raise FileNotFoundError(f"Integrated city data not found: {integrated_file}")
        
        df = pd.read_csv(integrated_file)
        
        # Create city_id if it doesn't exist
        if 'city_id' not in df.columns and 'city_name' in df.columns and 'province' in df.columns:
            df['city_id'] = df['city_name'] + '_' + df['province']
        
        self.logger.info(f"Loaded integrated city dataset: {df.shape[0]:,} rows, {df.shape[1]} columns")
        return df
    ...
    def validate_data_quality(self, df: pd.DataFrame) -> Dict[str, any]:
        ...
        if 'city_id' in df.columns:
            validation['spatial_coverage'] = {
                'unique_cities': int(df['city_id'].nunique()),
                'unique_provinces': int(df['province'].nunique()) if 'province' in df.columns else None,
                'observations_per_city': {
                    'mean': float(df.groupby('city_id').size().mean()),
                    'std': float(df.groupby('city_id').size().std())
                }
            }
        ...
        self.logger.info(f"  Spatial coverage: {validation['spatial_coverage']['unique_cities']:,} cities")
    ...
    def prepare_for_ml(self, df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
        ...
        for split_name, split_df in splits.items():
            self.logger.info(f"  {split_name}: {len(split_df):,} rows")
    ...
    def save_ml_ready_data(self, data_splits: Dict[str, pd.DataFrame], 
        ...
        output_dir = Path(__file__).parent.parent.parent / "data" / "processed" / "ml_analysis_city"
        ...
        metadata = {
            'dataset_info': {
                ...
                'spatial_coverage': validation['spatial_coverage']['unique_cities'],
    ...
    def load_and_prepare_ml_data(self) -> Dict[str, pd.DataFrame]:
        ...
        self.logger.info("Starting ML data preparation...")
        
        try:
            # Load city-level integrated dataset
            df = self.load_integrated_city_data()
            ...
            self.logger.info("ML data preparation completed successfully!")
            return data_splits
            
        except Exception as e:
            self.logger.error(f"Error during ML data preparation: {str(e)}")
            raise
...
def main():
    ...
    print("\nDatasets ready for ML analysis!")
    return data_splits
